classdef FIT_INFO
    ...
    methods
        ...
        function obj = buidFitParTable(obj)
            obj.model_table.material_type             = 'AlScZr_SANS_Linear';        % ASZ = 铝钪锆 Only for CoreShell
            ...
            obj.BO_opt_table.MaxObjectiveEvaluations  = 100;                          % 最大采样数：  10， 20 ，30 ，40 ，50
            obj.BO_opt_table.AcquisitionFunctionName  = 'expected-improvement-plus'; % 采样函数：    'expected-improvement', 'lower-confidence-bound'
            obj.BO_opt_table.NumSeedPoints            = 6000;                        % 初始种子点数
            obj.BO_opt_table.ExplorationRatio         = 0.4;                         % 外拓系数：    0.2, 0.4, 0.5, 0.6, 0.8 
            obj.BO_opt_table.IsObjectiveDeterministic = true;                        % 默认“True”
        % Options of Gradient Descent
        % Options of Grid Search
            obj.GS_opt_table.point_range  = 100;                                     % 搜索范围 ： 5 对应归一化后参数范围的 +- 0.005 
            obj.GS_opt_table.point_number = 50;                                     % 随机点选取数量
            obj.GS_opt_table.search_mode  = 'Random';                                % 如需要网格搜索模式 'No Random'
        % Program parameters
