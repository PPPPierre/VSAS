...
classdef VSAS_MAIN
    ...
    methods
        ...
        function item = VSAS_MAIN()
            item.window_width  = 1200;
            item.window_height = 850;
        ...
        function obj = initPanel(obj)
            ...
            obj.wd_res_fig = axes('Parent', obj.wd_main_window, ...
                                  'Position', [.37 .23 .55 .75], ...
                                  'Box', 'on', 'Tag', 'always');

            % 2. 创建"Select_mode"及下拉按钮
            uicontrol(obj.wd_main_window, ...
                      ...
                      'Position', [40 obj.window_height-40 100 30]);
            obj.wd_popmenu_mode = uicontrol(obj.wd_main_window, ...
                     'Style', 'popupmenu', ...
                     'Handlevisibility', 'On', ...
                     'Position', [150 obj.window_height-40 160 30], ...
                     'String', ['Choose|' ...
                                'Guinier approach|' ...
            ...
            uicontrol(obj.wd_main_window, ...
                      ...
                      'Fontname', obj.FONT_NAME, ...
                      'FontSize', obj.LABEL_FONT_SIZE, ...
                      'Position', [40 obj.window_height-65 100 20], ...
                      'Tag', 'm_choose_text', ...
                      'Visible', 'off');
            uicontrol(obj.wd_main_window, ...
                      'Style', 'pushbutton', ...
        ...
        function obj = loadData(obj, data)
            obj.dt_E_data             = data;
            obj.dt_q_max              = max(data(:, 1));
            obj.dt_E_max              = max(data(:, 2));
            obj.dt_E_min              = min(data(:, 2));
            obj.dt_E_stand_table.mean = mean(data(:,2));
            obj.dt_E_stand_table.std  = std(data(:,2));
            obj.dt_E_stand            = (data(:,2)-obj.dt_E_stand_table.mean)./obj.dt_E_stand_table.std;
        ...
        function obj = initDisFitPanel(obj)
            ...
            uicontrol(obj.wd_main_window, ...
                  ...
                  'Fontname', obj.FONT_NAME, ...
                  'FontSize', obj.LABEL_FONT_SIZE, ...
                  'position', [230 obj.window_height-65 50 25], ...
            ...
            uicontrol(obj.wd_main_window, ...
                  ...
                  'Fontname', obj.FONT_NAME, ...
                  'FontSize', obj.LABEL_FONT_SIZE, ...
                  'position', [320 obj.window_height-34 60 25], ...
                  'Callback', @settingsButtonCallback, ...
                  'Tag', 'm_button_settings_distr_fit', ...
                  'Visible', 'off');

            % 循环创建 Distribution fitting mode 模型设置面板
        ...
        function obj = showDisFitPanel(obj)
            ...
            %显示"Help、Start、Output"按钮
            obj.show('m_button_help_distr_fit');
            obj.show('m_button_settings_distr_fit');
        ...
        function obj = initGuinierPanel(obj)
            ...
            % 1. 创建"Porod fitting"面板，公式，"Start"、"Help"按钮
            ...
            uicontrol(fit_C_value_panel, ... 
                      ...
                      'Visible', 'off');
            % 2. 创建"Guinier fitting"面板，公式，"Remove Porod Intensity"及单选按钮，"Start"、"Help"按钮
            ...
            uicontrol(fit_Rg_panel, ...
                  ...
                  'Visible', 'off');
            % 3. 创建"Parameters"面板，"α、q_min、q_max"及滑动条、编辑框
            ...
            obj.wd_guinier_editTestqmax = uicontrol(parameters_panel, ...
                ...
                'Tag', 'm_edit_qmax', ...
                'Visible', 'off');
            % 4. 创建"Fit Results"面板，"C、R_g、r、Rg_sp、I_bg"
        ...
        function plotExpDataPoint(obj)
            ...
            axis([0 obj.dt_q_max+0.1 obj.dt_E_min-1 obj.dt_E_max+1])
        ...
        % 初始化 FIT_INFO 变量
        % 所有拟合相关的参数与数据都保存在 FIT_INFO 变量中
        % 这个变量是整个UI程序与拟合模块的桥接口
        % UI程序将用户的选择写入 FIT_INFO 变量中
        % 而拟合程序则从 FIT_INFO 变量中提取拟合所需要的参数
        function obj = buildVarINFO(obj)
            obj.FIT_INFO = obj.FIT_INFO.buildVarInfo(obj.dt_E_data, ...
                 obj.dt_E_stand, ...
                 obj.dt_E_stand_table, ...
                 obj.mode_dist, ...
                 obj.mode_form, ...
                 obj.mode_stru, ...
                 obj.mode_sld, ...
        ...
        % 绘制拟合I-q曲线
        function plotFitValue(obj)
            cla(obj.wd_res_fig);
            obj.plotExpDataPoint();
            fit_value = obj.getFitValue();
            plot(obj.wd_res_fig, obj.dt_E_data(:, 1), fit_value, 'r');
            hold on;
        end
